...
useEffect(() => {
  const fetchResults = async () => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/results/my`, {
        headers: {
...
const averageScore =
  ...
  return (
    <div className={`min-h-screen px-3 py-3 sm:px-4 md:p-12 transition-all duration-500 font-sans ${isDarkMode ? "bg-[#020617] text-slate-100" : "bg-white text-slate-900"}`}>
      <div className="max-w-6xl mx-auto space-y-10">
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-4 sm:gap-6">
        ...
        <div className={`overflow-hidden rounded-xl border ${
          ...
          <div className={`px-4 sm:px-6 md:px-8 py-4 sm:py-6 border-b flex justify-between items-center ${
            isDarkMode ? "border-blue-900/50 bg-transparent" : "border-blue-50 bg-blue-50/30"
          }`}>
            <h3 className={`text-xs sm:text-sm font-bold tracking-widest flex items-center gap-3 ${isDarkMode ? "text-blue-100" : "text-blue-900"}`}>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className={`${isDarkMode ? "text-blue-400/70" : "text-blue-600/70"} text-[10px] font-bold uppercase tracking-[0.15em]`}>
                  <th className="px-3 sm:px-5 md:px-8 py-3 sm:py-4">Assessment Module</th>
                  <th className="px-3 sm:px-5 md:px-8 py-3 sm:py-4 text-center">Efficiency</th>
                  <th className="px-3 sm:px-5 md:px-8 py-3 sm:py-4 text-right">Status</th>
                </tr>
              </thead>
              <tbody className={`divide-y ${isDarkMode ? "divide-blue-900/30" : "divide-blue-50"}`}>
                {results.map((r) => (
                  ...
                  >
                    <td className="px-3 sm:px-5 md:px-8 py-4 sm:py-6">
                      <div className="flex items-center gap-2 sm:gap-4">
                        <div className={`w-1.5 h-6 sm:h-8 rounded-full ${r.status === "Pass" ? "bg-sky-500" : "bg-rose-500"}`}></div>
                        <div>
                          <span className={`font-semibold text-xs sm:text-sm block transition-colors ${isDarkMode ? "text-slate-100 group-hover:text-blue-400" : "text-slate-900 group-hover:text-blue-600"}`}>{r.exam}</span>
                    ...
                    <td className="px-4 sm:px-6 md:px-8 py-4 sm:py-6 text-center">
                      <div className={`text-xs sm:text-sm font-bold tracking-tighter ${isDarkMode ? "text-slate-100" : "text-slate-900"}`}>{r.score} <span className={isDarkMode ? "text-slate-500" : "text-slate-400"}>/ {r.total}</span></div>
                    </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="p-12 space-y-8 self-center">
              <div className={`transition-all duration-700 ${animate ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'}`}>
                ...
                <p className={`text-xs sm:text-sm mt-2 font-medium ${isDarkMode ? "text-blue-300/60" : "text-blue-600/60"}`}>{exam.subject} Department •  Audit</p>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between group">
                  ...
                  <span className={`text-xs sm:text-sm font-bold uppercase tracking-widest ${isDarkMode ? "text-sky-400" : "text-sky-600"}`}>{exam.status}</span>
                </div>
                <div className="flex items-center justify-between group">
                  <div className="flex items-center gap-3">
                  ...
                  <span className={`text-xs sm:text-sm font-bold ${isDarkMode ? "text-slate-100" : "text-slate-900"}`}>{exam.score} <span className={isDarkMode ? "text-slate-500" : "text-slate-400"}>pts</span></span>
                </div>
              </div>
...
function StatBox({ title, value, icon: Icon, iconColor, isDarkMode }) {
  return (
    <div className={`p-4 sm:p-6 md:p-8 rounded-xl border transition-all ${
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className={`text-2xl sm:text-3xl md:text-4xl font-semibold tracking-tighter transition-colors ${isDarkMode ? "text-white" : "text-blue-900"}`}>{value}</p>
        </div>
